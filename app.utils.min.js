...
(function () {
  ...
  // — A) Tamanho de janela —
  // Em mobile desativada. Em desktop recalibra quando zoom muda.
  ...
  // — B) Image getter (do script original — detecta via console.log) —
  ...
  // Dispara a detecção continuamente via console interno
  // (o console.log foi silenciado para o usuário na seção 4,
  //  mas o getter ainda executa internamente)
  setInterval(function () {
    _devtoolsAberto; // mantém a referência viva
    console.log('%c', _imgTrap); // eslint-disable-line no-console
  }, 1000);

  // — C) Debugger timestamp — pega DevTools já aberto no carregamento —
  // Threshold em 500ms para não gerar falsos positivos em devices lentos.
  if (!_isMobile) {
    (function () {
      const t0 = Date.now();
      debugger; // pausa apenas se DevTools estiver aberto
      if (Date.now() - t0 > 500) {
        _devtoolsAberto = true;
      }
    })();
  ...
  //
  //  Checa rastros deixados por Playwright, Puppeteer, Selenium
  //  e scrapers comuns. Bloqueia antes de qualquer fetch.
  //  — plugins removido: causava falsos positivos em Firefox/TVs.
  // ============================================================
  (function _checarBot() {
    ...
    // Injetado automaticamente por Selenium / Playwright / Puppeteer
    if (navigator.webdriver === true) {
      _bloquearPagina('Acesso negado', 'Acesso negado.');
      return;
    }

    // Browsers reais sempre expõem languages; headless costuma omitir
  ...
  //     300 req/min: usuário legítimo nunca atinge.
  // ============================================================
  const _rateLimit = {
    janela: 60 * 1000,
    maxRequisicoes: 300,
  ...
  //     Ordem: velocidade suspeita → humano → rate limit.
  //     Sobrescrito UMA única vez — sem cadeia frágil.
  ...
  window.fetch = async function (url, opcoes = {}) {
    ...
    // 1. Velocidade suspeita (síncrono)
    _checarVelocidade();
