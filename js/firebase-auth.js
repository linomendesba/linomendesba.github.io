...
// ─── SESSÃO NO SERVIDOR ───────────────────────────────────
async function iniciarSessaoNoServidor(user) {
  try {
    ...
    console.error("Erro ao iniciar sessão no servidor:", e);
  }
}
...
function injectSubscriptionStyles() {
  ...
  style.textContent = `
    ...
    @keyframes sub-alerta-fade {
      0%, 100% { opacity: 1; }
      50%       { opacity: 0.55; }
    }

    #subscription-info.alerta {
      animation: sub-alerta-fade 1.6s ease-in-out infinite;
...
async function renderSubscriptionInfo(email) {
  ...
  waitForElement("#subscription-info", async (div) => {
    try {
      ...
      const alerta = dias !== null && dias >= 0 && dias <= 5;
...
onAuthStateChanged(auth, async (user) => {
  ...
  await iniciarSessaoNoServidor(user);
