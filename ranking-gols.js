...
const RankingGols = (() => {
  ...
  let currentChartMode = 'absoluto'; // 'absoluto' | 'acumulado' — lógica de construção dos pontos do gráfico
  let lastChartItems = []; // últimos items (top5 + fixados) montados pro gráfico, guardados pra poder re-renderizar só trocando a lógica sem reprocessar o ranking
  ...
  // ────────────────────────────────────────────────────────────────
  // GRÁFICO DE GOLS ACUMULADOS — 2ª LÓGICA (comparação com jogo anterior)
  // ────────────────────────────────────────────────────────────────
  // Diferente da lógica "Absoluta" acima (onde Y = gols daquele jogo,
  // sempre solto, sem relação com o jogo anterior), aqui o ponto é um
  // ACUMULADO: cada novo jogo soma ou subtrai do ponto anterior, e não
  // vira um valor novo isolado.
  //
  // Regra, jogo a jogo (a partir do 2º jogo da sequência):
  //   - diferença = gols do jogo atual − gols do jogo anterior
  //   - diferença positiva → o ponto SOBE essa quantidade
  //   - diferença negativa → o ponto DESCE essa quantidade
  //   - jogo atual com 0 gols → o ponto NÃO se move (fica no mesmo
  //     nível do ponto anterior), mesmo que o jogo anterior tenha
  //     tido gols — ou seja, 0 gol nunca "puxa" o gráfico pra baixo.
  //   Exemplo do pedido: jogo com 3 gols seguido de um jogo com 2 gols
  //   → diferença de 1 → o ponto desce 1.
  //
  // O 1º jogo da sequência não tem "jogo anterior" pra comparar, então
  // ele soma o próprio número de gols em cima do ponto inicial (0) —
  // mesmo ponto de partida da lógica Absoluta, só que a partir daí os
  // dois modelos passam a divergir.
  function buildGoalsWalkSeriesAcumulado(sequencia) {
    const points = [{ x: 0, y: 0, t: null, g: null }]; // ponto inicial, antes do primeiro jogo
    const colors = [GOL_NEUTRAL];
    let golsJogoAnterior = null;

    sequencia.forEach(({ golsTime, game }) => {
      const yAnterior = points[points.length - 1].y;
      let novoY;

      if (golsJogoAnterior === null) {
        // Primeiro jogo da sequência: sem base de comparação, soma o
        // próprio valor de gols em cima do ponto inicial.
        novoY = yAnterior + golsTime;
      } else if (golsTime === 0) {
        // Não marcou nesse jogo: mantém a posição atual, não desce.
        novoY = yAnterior;
      } else {
        const diferenca = golsTime - golsJogoAnterior;
        novoY = yAnterior + diferenca;
      }

      points.push({ x: points.length, y: novoY, t: formatGameTime(game), g: golsTime });
      colors.push(golsTime > 0 ? GOL_GREEN : GOL_RED);
      golsJogoAnterior = golsTime;
    });
  ...
  function renderTop5LineChart(items) {
    ...
    const seriesFn = currentChartMode === 'acumulado' ? buildGoalsWalkSeriesAcumulado : buildGoalsWalkSeries;

    const datasets = items.map((item, idx) => {
      const { points, colors } = seriesFn(item.sequencia || []);
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              ...
              label: (item) => {
                // "g" guarda sempre os gols reais daquele jogo — em modo
                // Acumulado, item.parsed.y é a posição no gráfico, não os
                // gols do jogo, então o texto tem que vir de "g".
                const raw = item.raw;
                const golsJogo = raw && raw.g !== null && raw.g !== undefined ? raw.g : item.parsed.y;
        ...
        scales: {
          ...
          y: {
            title: {
              display: true,
              text: currentChartMode === 'acumulado' ? 'Nível acumulado' : 'Gols no jogo',
              color: '#7a8499',
            },
            ticks: { stepSize: 1, color: '#7a8499', precision: 0 },
            grid: { color: 'rgba(255,255,255,0.05)' },
  ...
  // ────────────────────────────────────────────────────────────────
  // SELETOR DE LÓGICA DO GRÁFICO (Absoluto x Acumulado)
  // ────────────────────────────────────────────────────────────────
  const CHART_HINTS = {
    absoluto: '🟢 marcou · 🔴 não marcou · a linha sobe/desce conforme os gols de cada jogo · tracejada = gols do último jogo',
    acumulado: '🟢 marcou · 🔴 não marcou · o ponto sobe/desce conforme a diferença de gols pro jogo anterior · 0 gol não desce',
  };

  function updateChartHint() {
    const hint = document.getElementById('chartHint');
    if (hint) hint.textContent = CHART_HINTS[currentChartMode] || CHART_HINTS.absoluto;
  }

  function initChartModeSelect() {
    const select = document.getElementById('chartModeSelect');
    if (!select) return;
    select.value = currentChartMode;
    updateChartHint();

    select.addEventListener('change', () => {
      currentChartMode = select.value === 'acumulado' ? 'acumulado' : 'absoluto';
      updateChartHint();
      salvarFiltros();
      // Invalida a assinatura pra forçar recriar o Chart.js do zero —
      // necessário porque o título do eixo Y muda junto com a lógica,
      // e o caminho rápido de "mesma composição" só troca os dados.
      top5ChartSignature = [];
      renderTop5LineChart(lastChartItems);
    });
  }
  ...
  function restaurarFiltros() {
    ...
    try {
      ...
      currentChartMode = filtros.chartMode === 'acumulado' ? 'acumulado' : 'absoluto';
      ...
      if (chartModeSelect) chartModeSelect.value = currentChartMode;
      updateChartHint();
