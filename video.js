if (!window.__videoAccordionLoaded) {
    window.__videoAccordionLoaded = true;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('.video-accordion').forEach((content) => {
            const key = content.dataset.league;
            const url = LIGAS[key];
            const wrap = content.querySelector('.video-wrap');
            let carregado = false;

            if (!url) {
                console.warn('[video.js] Liga não encontrada em LIGAS:', key, content);
                return;
            }
            if (!wrap) {
                console.warn('[video.js] Não achei .video-wrap dentro do acordeon:', content);
                return;
            }
