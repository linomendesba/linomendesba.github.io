(function () {
  ...
  // Mesma validação usada no LigaStat (calculateMarketStats): só conta o jogo
  // se "ft" for string, contiver " x " e os dois lados derem número válido.
  // Sem isso, jogos com ft ausente/"OUT"/inválido viravam 0x0 e entravam
  // como falso positivo em empate/ambasNao/placar0x0, além de inflar o
  // denominador — foi isso que descolou a ⭐ dos % do LigaStat.
  function isFtValido(ft) {
    if (!ft || typeof ft !== 'string' || !ft.includes(' x ')) return false;
    const [a, b] = ft.split(' x ').map(Number);
    return !isNaN(a) && !isNaN(b);
  }

  function contarJogosValidos(slice) {
    let n = 0;
    for (const jogo of slice) if (isFtValido(jogo.ft)) n++;
    return n;
  }
  ...
  function calcMarketValue(slice, marketId) {
    ...
    let count = 0;
    for (const jogo of slice) {
      if (!isFtValido(jogo.ft)) continue; // ignora jogo sem placar válido, igual ao LigaStat
  ...
  function isVisivel() { return localStorage.getItem(LS_VISIVEL) !== '0'; } 
  ...
  function analisarEstrela(dadosCompletos, horas, mercado, gph) {
    ...
    // pct: denominador = jogos com placar válido (igual ao LigaStat: matches/validGames).
    // soma (golsFT/golsHT): mantém o tamanho da janela, como já era.
    const denom = info.tipo === 'pct' ? contarJogosValidos(janela) : janela.length;
    if (!denom) return null;
    const metrica = info.tipo === 'pct' ? (valor / denom) * 100 : (valor / denom);
    return { metrica, valor, jogos: denom };
  }
  ...
  function injetarCSS() {
    ...
    style.textContent = `
      ...
      .rdlg-wrap select {
        background: #333741; color: #7a8494;
        border: 1px solid rgba(148,163,184,.3);
        border-radius: 5px;
        font-size: 11px;
        padding: 2px 4px;
