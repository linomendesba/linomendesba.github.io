...
const LigaDataManager = (() => {
  ...
  async function fetchJson(url) {
    // Cache-busting: mesmo já tendo o cache em memória (getCached/CACHE_TTL_MS)
    // controlando quando uma requisição nova sai, a requisição em si ainda ia
    // pra URL idêntica todas as vezes — o que deixa ela sujeita ao cache HTTP
    // do navegador (ou de proxy/CDN na frente da API), podendo devolver uma
    // resposta velha sem nem chegar a bater na API de novo. Só um F5 "resolvia"
    // porque reload força renegociação de cache que o fetch() sozinho não força.
    // Aqui aplicamos a mesma técnica que o gráfico já usa (timestamp na query
    // string) + cache:"no-store" pra garantir que, quando o TTL expira e uma
    // requisição nova realmente sai, ela sempre chega fresca na API.
    const urlSemCache = url + (url.includes("?") ? "&" : "?") + `_ts=${Date.now()}`;
    const res = await fetch(urlSemCache, { cache: "no-store" });
