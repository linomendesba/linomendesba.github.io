...
const RankingMaxima = (() => {
  ...
  function renderProximosConfrontos() {
    ...
    // só o que mudou (horário, destaque de "quase máxima") é tocado, e
    // os cards que não mudaram nem saem do DOM.
  ...
  // os valores (posição, máxima, sequência) atualizados no lugar.
  function renderRankingRowsDiff(sorted) {
    ...
    sorted.forEach((item, index) => {
      ...
      if (row) {
        ...
        const maximaEl = row.querySelector('.maxima-value');
        if (maximaEl && maximaEl.textContent !== String(item.maxima)) maximaEl.textContent = item.maxima;

        const streakEl = row.querySelector('.streak-value');
        if (streakEl && streakEl.textContent !== String(item.streakAtual)) streakEl.textContent = item.streakAtual;
