/* ===========================================================
   BetStat — Ferramentas de desenho do gráfico (linha, zona, texto, etc.)
   Agora criado como INSTÂNCIA por painel (createInstance), não singleton global.

   FIX do bug "não desenha nada": antes o canvas de overlay era anexado como
   IRMÃO do container do gráfico (container.parentElement), num elemento que
   também continha a barra de ferramentas lateral — isso desalinhava as
   coordenadas do mouse com o gráfico. Agora o overlay é filho direto do
   próprio container do gráfico (position:relative + canvas absolute inset:0),
   então as coordenadas batem exatamente com o chart.
=========================================================== */
window.GraficoTradingTools = (function () {
    "use strict";

    function createInstance(root, getCore, storageKey) {
        let overlay = null;
        let ctx = null;
        let host = null;
        let dpr = 1;
        ...
        function core() { return getCore(); }
        function chart() { return core()?.getChart?.() || null; }
        function activeMarket() { return core()?.getActiveMarket?.() || "over25"; }
        function marketColor() { return core()?.getMarketColor?.(activeMarket()) || "#60a5fa"; }
        ...
        function ensureOverlay() {
            ...
            if (!overlay) {
                host = container; // <- FIX: overlay é filho do próprio container, não do pai dele
                overlay = document.createElement("canvas");
                overlay.className = "gt-overlay-canvas";
        ...
        function dispatchToolChange() {
            root.dispatchEvent(new CustomEvent("gt:toolchange", {
        ...
        function setTool(tool) {
            ...
            // "select" também precisa capturar ponteiro pra permitir
            // clicar e selecionar/apagar objetos já desenhados.
            const captureTools = ["select", "line", "vertical", "trend", "zone", "text", "delete"];
        ...
        function onPointerUp(e) {
            ...
            if (draft?.type === "trend") {
                if (validDraft(draft)) {
                    addObject({ type: "trend", x1: draft.x1, y1: draft.y1, x2: draft.x2, y2: draft.y2, color: draft.color });
                }
                draft = null;
                setTool("select");
                return;
            ...
            if (draft?.type === "zone") {
                if (validDraft(draft)) {
                    addObject({ type: "zone", x1: draft.x1, y1: draft.y1, x2: draft.x2, y2: draft.y2, color: draft.color });
                }
                draft = null;
                setTool("select");
            }
        ...
        function onKeyDown(e) {
            ...
            if ((e.key === "Delete" || e.key === "Backspace") && selectedId) {
                // evita apagar se o foco estiver num input de texto (edição de outro campo)
        ...
        function drawObject(obj, selected = false, preview = false) {
            ...
            if (obj.type === "trend") {
                const x1 = xFromLogical(obj.x1), y1 = yFromPrice(obj.y1);
                const x2 = xFromLogical(obj.x2), y2 = yFromPrice(obj.y2);
            ...
            if (obj.type === "text") {
                const x = xFromLogical(obj.x), y = yFromPrice(obj.y);
        ...
        function hitTest(p) {
            for (let i = objects.length - 1; i >= 0; i--) {
                ...
                if (obj.type === "text") {
                    const x = xFromLogical(obj.x), y = yFromPrice(obj.y);
                    if (Number.isFinite(x) && Number.isFinite(y) &&
                        Math.abs(p.x - x) <= 80 && Math.abs(p.y - y) <= 18) return obj;
                }
            }
            return null;
        }
        ...
        // botão de apagar / limpar tudo (shift+click)
        root.querySelector('[data-role="btnDelete"]')?.addEventListener("click", (e) => {
