/**
 * MARCADOR.JS — Ferramenta de anotação
 * Uso: <script src="marcador.js"></script>
 *      <button id="btnMarcador">Marcar</button>
 *      (o pai do elemento alvo precisa ter position: relative)
 *
 * Config opcional antes do script:
 *   window.MARCADOR_TARGET = '#meuElemento' // seletor do elemento alvo (padrão: body)
 *   window.MARCADOR_BTN    = '#meuBotao'    // seletor do botão (padrão: #btnMarcador)
 */
(function () {
  'use strict';
  ...
  const css = `
    ...
    #btnMarcador:hover {
      ...
      color: #26d4a7;
      box-shadow: 0 0 24px rgba(31,172,137,0.18);
    }
    ...
    #btnMarcador svg { width: 13px; height: 13px; flex-shrink: 0; }
    ...
    #mrcRoot.mrc-active #mrcBadge { opacity: 1; }
    ...
    #mrcRoot.mrc-active #mrcBtnFechar { opacity: 1; pointer-events: all; }
    ...
    #mrcRoot.mrc-active #mrcContador { opacity: 1; }
  ...
  // ── Injetar HTML no botão ─────────────────────────────────────────
  function init() {
    const btnSel    = window.MARCADOR_BTN    || '#btnMarcador';
    const targetSel = window.MARCADOR_TARGET || null;

    const btnAbrir = document.querySelector(btnSel);
    if (!btnAbrir) { console.warn('[Marcador] Botão não encontrado:', btnSel); return; }

    // ícone + texto no botão
    btnAbrir.innerHTML = svg.pencil + ' Marcar';

    // elemento alvo (onde o canvas vai por cima)
    const target = targetSel
      ? document.querySelector(targetSel)
      : btnAbrir.closest('[data-marcador-target]') || btnAbrir.parentElement;

    if (!target) { console.warn('[Marcador] Elemento alvo não encontrado.'); return; }
    ...
    root.innerHTML = `
      <canvas id="mrcCanvas"></canvas>
      <input id="mrcTextInput" type="text" placeholder="escreva… Enter pra confirmar">
      ...
      <button id="mrcBtnFechar">
        ${svg.close} sair
      </button>

      <div id="mrcContador">0 marcações</div>
      ...
      <div id="mrcToolbar">
        ...
        <button class="mrc-btn"            data-tool="retangulo" data-tip="Retângulo — R">${svg.rect}</button>
        <button class="mrc-btn"            data-tool="circulo"   data-tip="Círculo — C">${svg.circle}</button>
        ...
        <div class="mrc-color-wrap" data-tip="Cor">
          <input id="mrcColor" type="color" value="#1fac89">
        </div>
        <div class="mrc-size-wrap">
          <input id="mrcSize" type="range" min="1" max="40" value="3" data-tip="Espessura">
        </div>
        <div class="mrc-sep"></div>
    ...
    const colorEl   = root.querySelector('#mrcColor');
    const sizeEl    = root.querySelector('#mrcSize');
    const btnFechar = root.querySelector('#mrcBtnFechar');
    const btnDesfaz = root.querySelector('#mrcUndo');
    const btnLimpar = root.querySelector('#mrcLimpar');
    const textInput = root.querySelector('#mrcTextInput');
    const contador  = root.querySelector('#mrcContador');
    ...
    function atualizarContador() {
      contador.textContent = `${totalMarcacoes} marcaç${totalMarcacoes === 1 ? 'ão' : 'ões'}`;
    }
    ...
    root.querySelectorAll('[data-tool]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        root.querySelectorAll('[data-tool]').forEach(b => b.classList.remove('mrc-active'));
        btn.classList.add('mrc-active');
        canvas.style.cursor = ferramenta === 'borracha' ? 'cell' : 'crosshair';
      });
    });

    const getCor = () => colorEl.value;
    const getTam = () => parseInt(sizeEl.value);
    ...
    function desenharForma(x, y) {
      ...
      if (ferramenta === 'linha') {
        ...
        const ang = Math.atan2(y - startY, x - startX), hl = Math.max(14, getTam() * 4);
        ctx.beginPath(); ctx.moveTo(startX, startY); ctx.lineTo(x, y); ctx.stroke();
    ...
    function aoIniciar(e) {
      e.preventDefault();
      const { x, y } = getPos(e);
      if (ferramenta === 'texto') { abrirTexto(x, y); return; }
      desenhando = true; startX = x; startY = y;
      salvarHistorico(); aplicarEstilo();
      if (ferramenta === 'caneta' || ferramenta === 'borracha') { ctx.beginPath(); ctx.moveTo(x, y); }
    ...
    function aoMover(e) {
      ...
      const { x, y } = getPos(e); aplicarEstilo();
      if (ferramenta === 'caneta') { ctx.lineTo(x, y); ctx.stroke(); }
      else if (ferramenta === 'borracha') {
        ctx.globalCompositeOperation = 'destination-out';
        ctx.lineWidth = getTam() * 5; ctx.lineTo(x, y); ctx.stroke();
        ctx.globalCompositeOperation = 'source-over';
      } else desenharForma(x, y);
    }
    ...
    document.addEventListener('keydown', e => {
      ...
      const mapa = { p:'caneta', l:'linha', a:'seta', r:'retangulo', c:'circulo', t:'texto', e:'borracha' };
      if (mapa[e.key.toLowerCase()]) root.querySelector(`[data-tool="${mapa[e.key.toLowerCase()]}"]`)?.click();
      if (e.key === 'Escape') fechar();
      if (e.key === 'Delete') btnLimpar.click();
      if (e.ctrlKey && e.key === 'z') { e.preventDefault(); desfazer(); }
    });

    new ResizeObserver(() => { if (aberto) resizeCanvas(); }).observe(target);
    atualizarContador();
  }
