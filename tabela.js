// ═══════════════════════════════════════════════════════════════════════════════
// ROTAS DA LIGA — bloco autônomo, NÃO depende de config.js.
// Espelha exatamente as rotas que existem no backend (api.txt). Kiron Brasil e
// Kiron América Latina foram removidos porque essas rotas não existem na API
// (o backend só serve England/Italy/Spain pra Kiron).
// Esta página não precisa mais incluir <script src="config.js"> — só este bloco.
// ═══════════════════════════════════════════════════════════════════════════════
...
function detectarLigaAtual() {
  ...
  // BETANO
  if (caminho.includes("brasileirao.html"))         return "Brasileirão Betano";
  if (caminho.includes("campeonato_italiano.html"))  return "Campeonato Italiano";
  if (caminho.includes("copa_america.html"))         return "Copa América";
  if (caminho.includes("copa_das_estrelas.html"))    return "Copa das Estrelas";
  if (caminho.includes("euro.html") && !caminho.includes("bet365") && !caminho.includes("betsson")) return "Euro";
  if (caminho.includes("mundial.html"))              return "Mundial";

  // BET365
  if (caminho.includes("bet365copa.html"))    return "Bet365 Copa";
  if (caminho.includes("bet365euro.html"))    return "Bet365 Euro";
  if (caminho.includes("bet365super.html"))   return "Bet365 Super";
  if (caminho.includes("bet365premier.html")) return "Bet365 Premier";

  // BETSSON
  if (caminho.includes("betssonespanha.html"))  return "Betsson Espanha";
  if (caminho.includes("betssoningland.html"))  return "Betsson Inglaterra";
  if (caminho.includes("betssonbrasil.html"))   return "Betsson Brasil";

  // KIRON — só as 3 ligas que existem na API
  if (caminho.includes("kironengland.html")) return "Kiron Liga Inglaterra";
  if (caminho.includes("kironitaly.html"))   return "Kiron Liga Itália";
  if (caminho.includes("kironspain.html"))   return "Kiron Liga Espanha";

  // ESTRELABET
  if (caminho.includes("estrelacopamundo.html"))     return "Copa do Mundo";
  if (caminho.includes("estrelachampions.html"))     return "Ligas dos Campeões";
  if (caminho.includes("estrelaamericalatina.html")) return "América Latina";
...
// Cada entrada aponta pro segmento de rota real no backend (api.txt).
// "prefixo" ausente = rota direta tipo /resultados/<segmento> (caso Betano).
const ROTA_POR_LIGA = {
  // BETANO
  "Taça Glória Eterna":  { segmento: "Taça Glória Eterna" },
  "Copa América":        { segmento: "Copa América" },
  "Euro":                { segmento: "Euro" },
  "Campeonato Italiano": { segmento: "Campeonato Italiano" },
  "Copa das Estrelas":   { segmento: "Copa das Estrelas" },
  "Brasileirão Betano":  { segmento: "Brasileirão Betano" },
  "Mundial":             { segmento: "Mundial" },

  // BET365
  "Bet365 Copa":    { prefixo: "bet365", segmento: "Copa" },
  "Bet365 Euro":    { prefixo: "bet365", segmento: "Euro" },
  "Bet365 Super":   { prefixo: "bet365", segmento: "Super" },
  "Bet365 Premier": { prefixo: "bet365", segmento: "Premier" },

  // BETSSON
  "Betsson Espanha":    { prefixo: "betsson", segmento: "Espanha" },
  "Betsson Inglaterra": { prefixo: "betsson", segmento: "Inglaterra" },
  "Betsson Brasil":     { prefixo: "betsson", segmento: "Brasil" },

  // KIRON (England/Italy/Spain — as únicas ligas ativas na API)
  "Kiron Liga Inglaterra": { prefixo: "kiron", segmento: "England" },
  "Kiron Liga Itália":     { prefixo: "kiron", segmento: "Italy" },
  "Kiron Liga Espanha":    { prefixo: "kiron", segmento: "Spain" },

  // ESTRELA
  "Copa do Mundo":      { prefixo: "estrela", segmento: "Copa do Mundo" },
  "Ligas dos Campeões": { prefixo: "estrela", segmento: "Ligas dos Campeões" },
  "América Latina":     { prefixo: "estrela", segmento: "América Latina" },
};

function montarUrlLiga(tipo, nomeLiga) {
  const rota = ROTA_POR_LIGA[nomeLiga];
  if (!rota) {
    console.warn(`[rotas] Liga "${nomeLiga}" não mapeada em ROTA_POR_LIGA — usando fallback genérico.`);
    return `${API_BASE_URL}/${tipo}/${encodeURIComponent(nomeLiga)}`;
  }
  const caminho = rota.prefixo
    ? `${rota.prefixo}/${encodeURIComponent(rota.segmento)}`
    : encodeURIComponent(rota.segmento);
  return `${API_BASE_URL}/${tipo}/${caminho}`;
}

const ROTAS_API = {
  resultados:    (nomeLiga) => montarUrlLiga("resultados", nomeLiga),
  proximosJogos: (nomeLiga) => montarUrlLiga("proximos", nomeLiga),
  odds:          (nomeLiga) => montarUrlLiga("odds", nomeLiga),
};

// ═══════════════════════════════════════════════════════════════════════════════
// MINUTOS FIXOS POR LIGA — tabela estática (substitui a detecção automática em
// runtime). Chaveado pelo valor de LIGA_ATUAL definido acima, então cada
...
const MINUTOS_POR_LIGA = {
  ...
  "Campeonato Italiano":[2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35, 38, 41, 44, 47, 50, 53, 56, 59],
  "Copa das Estrelas":  [0, 3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36, 39, 42, 45, 48, 51, 54, 57],
...
let minutosFixos = (typeof LIGA_ATUAL !== "undefined" && MINUTOS_POR_LIGA[LIGA_ATUAL])
  ? MINUTOS_POR_LIGA[LIGA_ATUAL]
  : MINUTOS_FIXOS_PADRAO;
...
// ═══════════════════════════════════════════════════════════════════════════════
const HF_GALES_PADRAO = 2;
const HF_DIAS_FIXOS = 8;

function hfGalesKey() { return `hf_gales_${getLigaKey()}`; }
function hfHoraKey()  { return `hf_hora_${getLigaKey()}`; } // "atual" ou "0".."23"

function hfLerGales() {
  const v = parseInt(localStorage.getItem(hfGalesKey()), 10);
  return (v >= 1 && v <= 4) ? v : HF_GALES_PADRAO;
}
function hfSalvarGales(v) { localStorage.setItem(hfGalesKey(), String(v)); }

function hfLerHoraSelecionada() {
  return localStorage.getItem(hfHoraKey()) || "atual";
}
function hfSalvarHoraSelecionada(v) { localStorage.setItem(hfHoraKey(), v); }
...
function hfParseScore(s) {
  ...
  return p.length === 2 ? p : [0, 0];
}
function hfTotalGols(jogo) {
  const [g1, g2] = hfParseScore(jogo.ft);
  return g1 + g2;
}
...
function hfMontarSlotsHora(dados, horaAlvo, diasSelecionados) {
  ...
  const slots = {};
  minutosFixos.forEach(m => { slots[m] = {}; });
  const vistos = new Set();
  (dados || []).forEach(jogo => {
    if (jogo.hora !== horaAlvo || jogo.minuto == null || !jogo.data) return;
    const dia = getDateStr(jogo.data);
    if (!diasSet.has(dia)) return;
    const minNorm = minutosFixos.reduce((p, c) => Math.abs(c - jogo.minuto) < Math.abs(p - jogo.minuto) ? c : p);
    const chave = `${jogo.time_a}|${jogo.time_b}|${dia}|${minNorm}`;
    if (vistos.has(chave)) return;
    vistos.add(chave);
    slots[minNorm][dia] = jogo;
  });
  return slots;
}
...
function hfClassePct(p) {
  if (p >= 100) return "hf-100";
  if (p >= 90)  return "hf-90";
  if (p >= 80)  return "hf-80";
  if (p >= 70)  return "hf-70";
  if (p >= 60)  return "hf-60";
  if (p >= 50)  return "hf-50";
  if (p >= 40)  return "hf-40";
  return "hf-0";
}
...
function hfGarantirEstrutura() {
  ...
  if (!document.getElementById("hf-styles")) {
    ...
    st.textContent = `
      ...
      #hf-table-top td.hf-hora-cell { background: #292d36; color: #e5e7eb; font-weight: 700; width: 34px; }
...
function hfRender(dados) {
  ...
  const horaSelStr = hfLerHoraSelecionada();
  const { hora: horaAtualNumero } = qdGetHoraAtual(dados); // mesma identificação usada nos quadrantes
  const horaAlvo = horaSelStr === "atual" ? horaAtualNumero : parseInt(horaSelStr, 10);
  ...
  header.innerHTML = `<th style="width:34px;">H</th>` + minutosFixos.map(m => `<th>${m.toString().padStart(2, "0")}</th>`).join("");
  row.innerHTML = "";
  const tdHora = document.createElement("td");
  tdHora.className = "hf-hora-cell";
