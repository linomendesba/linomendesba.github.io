...
function getDateStr(d){if(d.includes("T")){const [datePart]=d.split("T");const[yy,mm,dd]=datePart.split("-");return`${yy}-${mm}-${dd}`;}const[dd,mm,yy]=d.split("/");return`${yy}-${mm}-${dd}`;}
...
async function buscarDados() {
  ...
  // Antes havia um atalho aqui que pulava a reconstrução da tabela quando
  // Estado.dadosMudaram() achava que "nada tinha mudado" — isso é o que
  // causava resultados presos até dar refresh (o diff por hash não pegava
  // certos casos). Agora sempre reconstrói do zero a cada ciclo de 5s;
  // criarTabela() já chama no final dela as mesmas funções que esse atalho
  // chamava (destaques, streak, cores, painel etc.), então não perde nada.
