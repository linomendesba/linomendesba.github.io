...
/* Prefixo 'kiron:' em TODAS as chaves — isola totalmente o armazenamento deste
   ...
   que compartilham o mesmo grafico.js mas usam as chaves 'mgraf:' sem prefixo. */
const SETUPS_KEY            = 'kiron:mgraf:setups_v4';
const ACTIVE_SETUP_KEY      = 'kiron:mgraf:activeSetup_v4';
const VISIBLE_DS_KEY        = 'kiron:mgraf:visibleDatasets_v4';
const DRAG_LINES_KEY        = 'kiron:mgraf:draglines';
const DRAG_FIBS_KEY         = 'kiron:mgraf:dragfibs';
const DRAG_TRENDS_KEY       = 'kiron:mgraf:dragtrends';
const ACCORDION_KEY         = 'kiron:mgraf:accordionOpen';
const SETUP_LINE_TOGGLE_KEY = 'kiron:mgraf:lineToggles_v4';
const Y_AXIS_POS_KEY        = 'kiron:mgraf:yAxisPosition';
...
/* Nome da liga atual — lido do elemento <h4 class="custom-color"> dentro de
   ...
   único por página. */
function _ligaAtual() {
    // Reaproveita o helper global getLigaKey() já usado no resto da página
    // (placar, times, odds etc.) para garantir a MESMA normalização de nome
    // de liga em todo o site.
    if (typeof window.getLigaKey === 'function') {
        try { const k = window.getLigaKey(); if (k) return k; } catch {}
    }
    ...
    if (nome) return nome.toLowerCase().replace(/\s+/g, '_');
...
function _captureControlsToSetup(setup) {
    ...
    // NOTA: linhas manuais, fibonacci livre e linhas de tendência (LTA/LTB)
    // NÃO fazem parte do setup — são anotações visuais atreladas à liga atual
    // (ver _loadDragLines/_saveDragLines etc., já isoladas por _ligaKey()).
    // Guardá-las aqui fazia com que trocar de liga com um setup customizado
    // ativo reaplicasse as linhas da liga anterior na liga nova.
...
function _applySetup(setup) {
    ...
    // Linhas manuais / fibonacci livre / tendências (LTA/LTB) NÃO pertencem ao
    // setup — são da liga atual. Ao aplicar um setup, recarrega essas anotações
    // sempre a partir do armazenamento isolado por liga (_ligaKey), nunca do
    // setup, para não vazar entre ligas.
    const ci = chartInstances['Copa'] || Object.values(chartInstances)[0];
    if (ci) {
        ci.dragLines = _loadDragLines();
        ci._selectedDragLine = -1;
        _atualizarContador(ci.dragLines);

        ci.fibDraws = _loadDragFibs();
        ci._selectedFib = -1;
        _atualizarContadorFibs(ci.fibDraws);

        ci.trendLines = _loadDragTrends();
        ci._selectedTrend = -1;
...
function processApiData(data, league) {
    // Ignora jogos registrados às 3h da manhã: a kiron nunca tem dados/mercados
    ...
    // Pega jogos suficientes: numPoints exibidos + averagePoints jogos extras
    // pra preencher a janela de média do primeiro ponto exibido. Contagem exata
    // de jogos, sem depender de "minutos por jogo" — não quebra com atraso/gap.
    ...
    // Janela de média por CONTAGEM de jogos (igual ao Modo Trader), não por tempo real.
    // "Base 30" = sempre os 30 jogos anteriores, não importa quantos minutos/horas
    // eles cobrem no relógio — isso elimina qualquer mergulho falso causado por
    // horários com poucos ou nenhum jogo (3h da manhã, atrasos, manutenção, etc.).
    const janelaJogos = averagePoints + 1;
    ...
    for(let i=0;i<slicedData.length;i++){
        ...
        for(let j=i;j>=0;j--){
            if(i - j >= janelaJogos) break; // já pegou os últimos "Base 30" jogos por contagem
